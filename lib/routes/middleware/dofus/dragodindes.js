...
import dofusInfosModel from './../../../models/dofus_infos.js';
...
const getOrSetNotificationsByStatus = async (req, res, next) => {
    if (res.allDofusInfos && req.body.status) {
        const status = await dofusInfosModel.setNotificationsByStatus(req.allDofusInfos, req.body.status.toLowerCase());
        if (status) {
            res.dragodindes = true;
        }
    }
    else if (res.allDofusInfos && req.body.method === 'get') {
        res.dragodindes = res.allDofusInfos.notif;
    }
    next();
};
...
exports.getAllDragodindesNotifInfos = getAllDragodindesNotifInfos;
exports.getDofusInfos = getDofusInfos;
exports.getDragodindes = getDragodindes;
exports.getOrSetNotificationsByStatus = getOrSetNotificationsByStatus;
