...
describe('SETTINGS', () => {
    ...
    describe('Model', () => {
        ...
        it('updateSettings() => Return updated settings', async () => {
            ...
            expect(Object.keys(settings)).to.be.an('array').that.have.lengthOf(6);
            expect(settings.guildId).to.equal(settingsObj.guildId);
            expect(Object.keys(settings.notif)).to.deep.equal(Object.keys(updatedSettings.notif));
            expect(Object.values(settings.notif)).to.deep.equal(Object.values(updatedSettings.notif));
            expect(Object.keys(settings.audio)).to.deep.equal(Object.keys(updatedSettings.audio));
            expect(Object.values(settings.audio)).to.deep.equal(Object.values(updatedSettings.audio));
            expect(noGuildId).to.be.false;
            expect(noNotif).to.be.false;
            expect(noAudio).to.be.false;
        });
